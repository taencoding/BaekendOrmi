...
[1, 3, 4, 8, 13, 17, 20]
[3, 4, 8, 13, 17, 20]
// python에서 sort를 했을 때 주었던 key function를 여기서 구현해볼 겁니다.

const zip = (a, b) => a.map((v, i) => [v, b[i]])
let s = [1, 3, 4, 8, 13, 17, 20]

sip(s, s.slice(1))
sip(s, s.slice(1)), sort((a, b) => (a[1] - a[0]) - (b[1] - b[0]))
sip(s, s.slice(1)), sort((a, b) => (a[1] - a[0]) - (b[1] - b[0]))[0]
...
// 최소값을 찾아내려면 먼저 최대값을 저장해라!
// 최대값을 찾아내려면 먼저 최소값을 저장해라!
let s = [1, 3, 4, 8, 13, 17, 20]
let min = Infinity
for (let i = 0; i < s.length - 1; i++) {
    if (s[i + 1] - s[i] < min) {
...
// 다음 친구들이 엘리베이터를 탔을 때 최대로 탑승할 수 있는 인원 수를 구하세요.
